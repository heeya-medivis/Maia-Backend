...
export const maiaSessions = pgTable('maia_sessions', {
  ...
  totalInputTextCachedTokens: integer('total_input_text_cached_tokens').default(0).notNull(),
  totalInputImageCachedTokens: integer('total_input_image_cached_tokens').default(0).notNull(),
  totalInputAudioCachedTokens: integer('total_input_audio_cached_tokens').default(0).notNull(),
  totalOutputTextTokens: integer('total_output_text_tokens').default(0).notNull(),
  totalOutputImageTokens: integer('total_output_image_tokens').default(0).notNull(),
  totalOutputAudioTokens: integer('total_output_audio_tokens').default(0).notNull(),
  totalOutputReasoningTokens: integer('total_output_reasoning_tokens').default(0).notNull(),
...
export const maiaSessionTurns = pgTable('maia_session_turns', {
  ...
  // Cached input tokens
  inputTextCachedTokens: integer('input_text_cached_tokens').default(0).notNull(),
  inputImageCachedTokens: integer('input_image_cached_tokens').default(0).notNull(),
  inputAudioCachedTokens: integer('input_audio_cached_tokens').default(0).notNull(),
  ...
  outputReasoningTokens: integer('output_reasoning_tokens').default(0).notNull(),
...
// Relations
export const maiaSessionsRelations = relations(maiaSessions, ({ one, many }) => ({
  user: one(users, {
...
export const maiaSessionTurnsRelations = relations(maiaSessionTurns, ({ one }) => ({
