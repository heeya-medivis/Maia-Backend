...
export function OrganizationDetailContent({ organizationName }: OrganizationDetailContentProps) {
  ...
  const { data, isLoading, error, refetch } = useApi(
    () => api.getAdminOrganizationDetail(organizationName),
    [organizationName]
  );
  ...
  const allTimeStats = useMemo(() => {
    if (!data) return { chatCount: 0, analysisCount: 0, totalTokens: 0, userCount: 0 };
  ...
  if (error || !data) {
    return (
      <div className="text-center py-12">
        <p className="text-red-400">Error loading organization data: {error?.message || 'Organization not found'}</p>
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-[var(--muted)]">
              {allTimeStats.userCount} {allTimeStats.userCount === 1 ? 'user' : 'users'} in this organization
            </p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
        <Card className="p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xl font-bold">{allTimeStats.userCount}</p>
            </div>
          </div>
        </Card>
        ...
        <Card className="p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm text-[var(--muted)]">Analyses</p>
      ...
      <Card className="p-4">
        ...
        <div className="flex flex-wrap gap-2">
          {data.users.map((user) => (
            <button
              key={user.id}
              onClick={() => router.push(`/admin/sessions/user/${user.id}`)}
              className="px-3 py-2 bg-[var(--card-hover)] rounded-lg hover:bg-[var(--accent)]/20 transition-colors text-sm"
            >
              {user.firstName} {user.lastName}
              <span className="text-[var(--muted)] ml-1">({user.email})</span>
            </button>
          ))}
        </div>
      </Card>
